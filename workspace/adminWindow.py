...
class WorkspaceAdmin:
    def __init__(self,root,uniqid):
        ...
        self.showFrame = LabelFrame(self.root, text="Database",
            labelanchor='n',bg=BackgroundColor.background).grid(
            row=0, column=0,sticky=E+W+N+S,padx=10,pady=10)

        self.searchFrame = LabelFrame(self.root, text="Values",
            labelanchor='n',bg=BackgroundColor.background).grid(
            row=1, column=0,sticky=E+W+N+S,padx=10,pady=10)

        self.changeFrame = LabelFrame(self.root, text="Operation",
            labelanchor='n',bg=BackgroundColor.background).grid(
            row=2, column=0,sticky=E+W+N+S,padx=10,pady=10)

        # Treeview

        self.treeFrame = ttk.Treeview(self.showFrame, selectmode="browse",
            columns=(1,2,3),show="headings",height="6").pack(side="left")

        self.verscrlbar = ttk.Scrollbar(self.showFrame, orient ="vertical", 
            command = self.treeFrame.yview).pack(side ='right', fill ='y')
        ...
        self.treeFrame.heading("3", text ="NUMBER")
    ...
    def show(self, Object, uniqueId:str):
        with Database(self.connectorDetail) as con:
            ...
            cursor.execute(
                ...
                .format(self.connectorDetail.database,self.uniqid))
